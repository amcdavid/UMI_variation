...
baseDir <- "/net/irizarryfs01/srv/export/irizarryfs01_backed_up/share_root/shicks/papers/zeisel"
...
Here we download the data, create an `SCESet` object. 
Data uses UMIs, so we use counts per million (CPM) in `exprs` slot as transcript length is irrelevant. 
...
pdf(file.path(baseDir, "figs/MeanVar_relationship.pdf"), width = 6, height = 6)
par(mfrow=c(1,1))
...
pdf(file.path(baseDir, "figs/VarandGCcontent_relationship.pdf"), width = 12, height = 5)
par(mfrow=c(1,3))
